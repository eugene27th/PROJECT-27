...
_idap_vehicle = selectRandom idap_vehicles_array createVehicle _pos;
_idap_vehicle setDir _direction;

_enemy_vehicle_pos = [_pos, 10, 100, 5, 0] call BIS_fnc_findSafePos;
_enemy_vehicle = selectRandom (enemy_cars_array + enemy_heavy_armed_vehicle_array) createVehicle _enemy_vehicle_pos;
_enemy_vehicle_crew = [_enemy_vehicle,enemy_units_array,true] call prj_fnc_create_crew;

_idap_units = [];
_enemies = [];

for "_i" from 1 to 2 do {
	private _unit = (createGroup civilian) createUnit [selectRandom idap_units_array, _unit_pos, [], 0, "NONE"];
...
for [{private _i = 0 }, { _i < ([10,15] call BIS_fnc_randomInt) }, { _i = _i + 1 }] do {
	...
	private _unit = (createGroup independent) createUnit [selectRandom enemy_units_array, _position, [], 0, "NONE"];
