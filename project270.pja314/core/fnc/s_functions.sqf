...
prj_fnc_raid = {
	...
	if (isNil "_pos") then {
		...
		{
			if (_x distance spawn_zone > 100 || (random 1) < 0.2) then {
	...
	private _spawnPos = [_pos, [600,700] call BIS_fnc_randomInt, [0,359] call BIS_fnc_randomInt] call BIS_fnc_relPos;

	for [{ private _i = 0 }, { _i < _unitInt }, { _i = _i + 1 }] do {
		private _unit = _grp createUnit [selectRandom enemy_infantry, _spawnPos, [], 0, "NONE"];

		if (dayTime >= 19 || dayTime < 5) then {
			[_unit, 4] call WBK_LoadAIThroughEden;
		};
	...
	// ["radioIn"] remoteExec ["playSound", 0];
	// [[west, "AirBase"], format["Наблюдаем движение неподалёку от %1.",_gridPos]] remoteExec ["sideChat"];

	[_enemy, _object, _wp, _grp] spawn {
		params ["_units", "_object", "_wp", "_grp"];

		while {(count _units) > 0} do {
			uiSleep 180;
			
			private _unitsCount = (count _units) - 1;
			private _oldWp = _wp;
			private _deleteArr = [];
			...
			for "_i" from 0 to _unitsCount do {
				...
				if (alive (_units # _i)) then {
					if (!isNil "mhqterminal") then {
						if ((mhqterminal distance (_units # _i)) > 1500) then {
							{
								if ((_x distance (_units # _i)) > 1500) exitWith {
									_deleted = true;
								};
							} forEach allPlayers;
						};
				...
				if (_deleted) then {
					_deleteArr pushBack (_units # _i);
					deleteVehicle (_units # _i);
				};
			};

			if !(_object isEqualTo []) then {
				private _newWp = _grp addWaypoint [position _object, 0];
				_newWp setWaypointType "SAD";
				deleteWaypoint _oldWp;
				_oldWp = _newWp;
			};

			{_units deleteAt (_units find _x)} forEach _deleteArr;
		};
	};
};
...
prj_fnc_createCheckpoint = {
	...
	if (dayTime > 5 && dayTime < 20) then {
		_crewUnits = [_barricadingVehicle,enemy_infantry] call prj_fnc_createCrew;
...
prj_fnc_createSentryPatrol = {
	...
	if (enemy_heliSentry isEqualTo [] || (!_forced && (random 1) < _chance)) exitWith {
		[_sentryPos] call prj_fnc_createReinforcement;
	};
	...
	[_vehicle,_vehUnits] spawn {
		...
		while {alive _vehicle} do {
			...
			if (!isNull _nearEnemy) exitWith {
				...
				switch (_enemyCategory) do {
					case "Soldier": {
						[position _nearEnemy,2,"antiInf"] call prj_fnc_createReinforcement;
					};
					case "Vehicle": {
						if (_enemyType isEqualTo "Helicopter" || _enemyType isEqualTo "Plane") then {
							[position _nearEnemy,2,"antiAir"] call prj_fnc_createReinforcement;
						} else {
							if (_enemyType isEqualTo "Car") then {
								[position _nearEnemy,2,"antiInf"] call prj_fnc_createReinforcement;
							} else {
								[position _nearEnemy,2,"antiTank"] call prj_fnc_createReinforcement;
							};
						};
					};
				};
		...
		if (!alive _vehicle) then {
			...
			[_heliCrashPos,2,"antiInf"] call prj_fnc_createReinforcement;
		};
	};
	...
	switch (_type) do {
		...
		case "groundToAir": {
			if (enemy_vehicles_aa isEqualTo []) exitWith {
				[_pos,2,"groundToInf"] call prj_fnc_createReinforcement;
			};
		...
		case "airToGround": {
			if (enemy_heliHeavy isEqualTo []) exitWith {
				[_pos,3,"groundToGround"] call prj_fnc_createReinforcement;
			};
		...
		case "airToAir": {
			if (enemy_fighters isEqualTo []) exitWith {
				[_pos,2,"groundToAir"] call prj_fnc_createReinforcement;
			};
...
prj_fnc_zoneCapture = {
	...
	if (dayTime > 5 && dayTime < 19) then {
		[_trigger_pos] call prj_fnc_createReinforcement;
		[_trigger_pos] call prj_fnc_createSentryPatrol;
	} else {
		[_trigger_pos] spawn {
			params ["_trigger_pos"];
			for "_i" from 1 to 3 do {
				[_trigger_pos, 10] call prj_fnc_raid;
				uiSleep 10;
			};
		};
	};
