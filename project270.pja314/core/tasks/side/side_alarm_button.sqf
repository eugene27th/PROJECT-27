...
private _enemy_vehicle_crew = [];

if (dayTime > 5 && dayTime < 19) then {
    _enemy_vehicle_crew = [_enemy_vehicle,enemy_infantry,true] call prj_fnc_createCrew;
};
...
if (dayTime >= 19 || dayTime < 5) then {
    ...
    (_idap_units # 0) moveInDriver _idap_vehicle;
    (_idap_units # 1) moveInAny _idap_vehicle;
};
