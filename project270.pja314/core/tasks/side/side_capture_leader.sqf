...
private _leader = (createGroup [enemySide, true]) createUnit [selectRandom enemy_leaders, _pos, [], 0, "NONE"];
private _enemy = (createGroup [enemySide, true]) createUnit [selectRandom enemy_infantry, position _leader, [], 0, "NONE"];
...
if (dayTime > 5 && dayTime < 19) then {
	_leader addEventHandler ["FiredNear", {
		...
		[position _unit] call prj_fnc_createReinforcement;
	}];
};

[_taskID + "_blue_base",position spawn_zone,"ColorWEST",0.7,[[50,50],"ELLIPSE"]] call prj_fnc_createMarker;
[_taskID + "_center",_center_pos,"ColorEAST",0.7,[[200,200],"ELLIPSE"]] call prj_fnc_createMarker;
